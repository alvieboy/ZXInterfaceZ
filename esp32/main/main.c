...
static void set_clear_flags(uint8_t enable, uint8_t disable);
int strtoint(const char *str, int *dest);


static inline void set_flags(uint8_t enable)
{
    set_clear_flags(enable, 0);
}
static inline void clear_flags(uint8_t disable)
{
    set_clear_flags(0, disable);
}
...
static const char *TAG = "ZX InterfaceZ";
...
static void get_framebuffer(uint8_t *target);
...
static uint32_t getbe32(const uint8_t *source)
{
...
{
    // Stop, clear
    set_clear_flags( FLAG_CAPCLR, FLAG_CAPRUN);
    set_clear_flags( FLAG_CAPRUN, 0);
}

void start_capture()
{
    // Reset spectrum, stop capture
    set_clear_flags( FLAG_RSTSPECT, FLAG_CAPRUN );
    // Reset fifo, clear capture
    set_clear_flags( FLAG_CAPCLR | FLAG_RSTFIFO, 0 );

    // Remove resets, start capture
    set_clear_flags( FLAG_CAPRUN | FLAG_ENABLE_INTERRUPT , FLAG_RSTFIFO| FLAG_RSTSPECT| FLAG_CAPCLR );
